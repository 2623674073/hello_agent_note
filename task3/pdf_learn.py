...
def create_gradio_ui():
    ...
    def load_pdf(pdf_file) -> str:
        ...
        # `type="filepath"` returns a path string in Gradio 4.  Keep the
        # object fallback for callers that provide a file-like value.
    ...
    with gr.Blocks(title="智能文档问答助手", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("🏠 开始使用"):
            ...
            # Gradio 4.44's API schema parser cannot parse the newer
            # FileData/Chatbot JSON schema.  These callbacks are UI-only, so
            # keep them out of the generated API documentation.
            load_btn.click(
                load_pdf,
                inputs=[pdf_upload],
                outputs=[load_output],
...
def main():
    ...
    demo.launch(
        # Bind locally by default.  Gradio's localhost health check is more
        # reliable than checking a wildcard address, especially when a proxy
        # is configured in the shell environment.
